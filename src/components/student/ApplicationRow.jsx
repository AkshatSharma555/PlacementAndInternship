...
const ApplicationRow = ({ application }) => {
  ...
  const baseButtonStyles = "w-full text-center text-sm px-3 py-1.5 rounded-md transition-colors";

  return (
    <div className="bg-white p-4 rounded-xl shadow-md grid grid-cols-7 gap-4 items-center mb-4">
      {/* Role */}
      <div className="col-span-1">
        <p className="font-bold text-gray-800">{application.title}</p>
      ...
      <div className="col-span-1 flex flex-col gap-2">
        ...
        <button className={`${baseButtonStyles} border border-gray-300 text-gray-700 hover:bg-gray-100`}>View JD</button>
        <button className={`${baseButtonStyles} bg-red-100 text-red-700 hover:bg-red-200`}>Withdraw</button>
      </div>
    </div>
  );
};
